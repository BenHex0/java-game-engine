...
public class Enemy extends Entity {
    ...
    private void pathFindingMove() {
        ...
        double px = level.getPlayer().getPviot().getX();
        double py = level.getPlayer().getPviot().getY();
        ...
        // --- (3. Execute Movement) ---
        if (xAxis != 0 || yAxis != 0) {
            // Ensure move(xAxis, yAxis) moves by a speed (e.g., 1 or 2 pixels)
            // that allows the NPC to land exactly on a tile boundary (x % 16 == 0).
